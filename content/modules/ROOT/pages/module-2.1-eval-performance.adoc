...
First, clone the benchmark pipeline repo, apply the PVC, task, and pipeline. We'll also create an s3 bucket in Minio where the pipeline will upload the benchmark results.

[source,console,role=execute,subs=attributes+]
----
https://github.com/jhurlocker/guidellm-pipeline.git 
----
